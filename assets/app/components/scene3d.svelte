...
onMount(() => {
    ...
    ModelHelper.createPoster(scene, '../posters/infos.png', {x: 13.2, y: -1.96, z: -1.96}, -1.575, "posterContact", cameraMoveContact, {width: 1.18, height: 3.2});
    ModelHelper.createPoster(scene, '../posters/talents.png', {x: 13.2, y: -0.04, z: -4.05}, -1.575, "posterContact", cameraMoveContact, {width: 1.18, height: 3.35});
    const poster3 = ModelHelper.createPoster(scene, '../posters/affiche.png', {x: 13.2, y: -0.26, z: -2.28}, -1.58, "posterContact", cameraMoveContact, {width: 1.18, height: 3.35});
    const grandPoster = ModelHelper.createPoster(scene, '../posters/affiche.png', {x: 11.5, y: -1.3, z: -6.9}, 0, "posterContact", cameraMoveContact, {width: 1.93, height: 5.32});
    const grandPosterEntree = ModelHelper.createPoster(scene, '../posters/affiche.png', {x: 4.8, y: -1.64, z: -7.95}, 0, "posterContact", cameraMoveContact, {width: 1.38, height: 3.8});
    ...
    ModelHelper.createPoster(scene, '../posters/affiche.png', {x: 5.15, y: -2.34, z: 4.45}, 0.1, "posterContact", cameraMoveContact, {width: 1.38, height: 3.8});
    ModelHelper.createPoster(scene, '../posters/affiche.png', {x: 9.25, y: -2.33, z: 4.3}, 0, "posterContact", cameraMoveContact, {width: 1.38, height: 3.85});
    const grandPosterEntree2 = ModelHelper.createPoster(scene, '../posters/affiche.png', {x: -9.02, y: -2.34, z: 4.2}, 0, "posterContact", cameraMoveContact, {width: 1.38, height: 3.85});
    ModelHelper.createPoster(scene, '../posters/affiche.png', {x: -5.14, y: -2.33, z: 4.45}, -0.07, "posterContact", cameraMoveContact, {width: 1.38, height: 3.85});
    
    poster3.rotation.x = 1.58
    poster4.rotation.x = 1.58
    posterTalent.rotation.x = 1.58;

    // const cubeTestRotation = SceneHelper.gui.addFolder('floor ')
    //     cubeTestRotation.add(floor.position, 'x').min(-60).max(60).step(0.01).name('position X')
    //     cubeTestRotation.add(floor.position, 'y').min(-60).max(60).step(0.01).name('position Y')
    //     cubeTestRotation.add(floor.position, 'z').min(-60).max(60).step(0.01).name('position Z')
    //     cubeTestRotation.add(floor.rotation, 'x').min(-60).max(60).step(0.01).name('rotation X')
    //     cubeTestRotation.add(floor.rotation, 'y').min(-60).max(60).step(0.01).name('rotation Y')
    //     cubeTestRotation.add(floor.rotation, 'z').min(-60).max(60).step(0.01).name('rotation Z')
    ...
    const returnButton1 = ModelHelper.createButton(scene, "", 1.5, {x: 10.1, y: -0.07, z: -5.55}, "return1", cameraMoveReturn);
